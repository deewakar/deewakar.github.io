...
** Other essential standard modules
   ...
   - datetime, array, ctypes and struct (data types for system programming)
   - os.path, subprocess, threading, multiprocessing and asyncio (process handling and I/O)
   - socket, json, http.server, http.client, http, html.parser (networking and web) 
   - wave (wav audio file), zlib, hashlib (useful file types and crypto)
   - pydoc, pdb, timeit, trace, cProfile, distutils, venv and logging (python programmer support)
